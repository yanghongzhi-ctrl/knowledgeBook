...
DEFAULT_OUTPUT_JSON = ROOT / "output" / "ch11_p2_quick_source_review_packet_2026-06-05.json"
DEFAULT_OUTPUT_MD = ROOT / "output" / "ch11_p2_quick_source_review_packet_2026-06-05.md"
DEFAULT_OUTPUT_CSV = ROOT / "output" / "ch11_p2_quick_source_review_packet_2026-06-05.csv"


def main() -> int:
    parser = argparse.ArgumentParser(description="Build a focused P2 quick-confirm review packet for ch11 Source_Chunks.")
    ...
    decisions = [
        ...
        and item.get("priority_group") == "P2_ch11_quick_confirm"
    ]
    ...
    payload = {
        ...
        "chapter_id": "ch11",
        "priority_group": "P2_ch11_quick_confirm",
        "purpose": "Focused P2 quick-confirm packet for ch11 candidate anchors. Suggestions are not manual approvals.",
        "items": items,
        "summary": {
            "total": len(items),
            "filled": filled,
...
def review_prompt(item: dict[str, Any]) -> str:
    if item.get("review_status") == "candidate_anchor_review":
        return (
            "核对 Source_Chunk 与候选 Word 段落是否表达同一概念。若可支撑，使用 "
            "confirm_candidate_anchor；若不充分，使用 manual_anchor_pending 或 reject_candidate。"
        )
